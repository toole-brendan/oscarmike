...
import { exerciseInfo } from '@shared/schema';

const Pullups: React.FC = () => {
  const [_, navigate] = useLocation();
  const { toast } = useToast();
  
  const exerciseType = 'pullups';
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      const exerciseData = {
        ...
        runTime: null,
        completedAt: new Date(), // Use Date object directly instead of ISO string
  ...
  const calculatePoints = () => {
    ...
    let points = Math.min(100, repCount * 5);
        ...
        <Card className="p-6 flex flex-col">
          <div className="mb-6">
            ...
            <p className="text-sm text-gray-600">
              Complete as many correct pull-ups as possible in the 2-minute time period.
            </p>
          </div>
